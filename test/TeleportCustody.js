...
contract('TeleportCustody (ICONS) Tests', (accounts) => {
  let teleportCustody;
  ...
  beforeEach('setup contract for each test', async () => {
    teleportCustody = await TeleportCustodyTest.new(IconsTokenTest.address);
  });
  ...
  describe('TeleportCustody', () => {
    describe('lock()', () => {
      it('can lock tokens from users', async () => {
        ...
        const transferEvents = rawLogs.filter(event => event.address === IconsTokenTest.address);
